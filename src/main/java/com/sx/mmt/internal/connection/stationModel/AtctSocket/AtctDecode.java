...
public class AtctDecode  extends CumulativeProtocolDecoder{
	...
	protected boolean doDecode(final IoSession session, final IoBuffer buffer,
		...
		if (context.getDecodeStatus() == DecodeStatus.START) {
			doDecodeATCT(context, buffer,out);
		}
		if (context.getDecodeStatus() == DecodeStatus.ATCT) {
	...
	private void doDecodeATCT(final Context context,
			final IoBuffer buffer,final ProtocolDecoderOutput out) {
		if(buffer.remaining() >=8){
			byte[] atct=new byte[8];
			buffer.get(atct);
			String atctString=new SimpleBytes(atct).toAscii();
			String tag=atctString.substring(0, 4);
			if("ATCT".equals(tag) || "EROR".equals(tag)){
				
				out.write(atctString);
				context.setDecodeStatus(DecodeStatus.NEXT);
			}else{
				buffer.position(buffer.position() - 8);
				context.setDecodeStatus(DecodeStatus.ATCT);
			}
		}
	}
