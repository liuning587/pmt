...
public class CQDecode extends CumulativeProtocolDecoder{
	...
	private void doDecodePacket(Context context, IoBuffer buffer,
		...
		while(buffer.hasRemaining()){
			try{
				...
				if(b=='>' && 
						(context.getLastFiveString().equals("sult>") ||
								context.getLastFiveString().equals("tion>"))){
					SimpleBytes p=context.getPacket();
					String recode=new String(p.getBytesArray(),charset);
					if(recode.endsWith("</result>") ||
							recode.endsWith("</exception>")){
						recode=recode.substring(recode.indexOf('<'));
						InputStream in=new ByteArrayInputStream(recode.getBytes(charset));
