...
public class CustomMessageCommand extends SinglePacketCommand<CustomMessageCommand>{
	@Override
	public void sendOrder(String from, String to, String event, DecodedPacket context) throws Exception {
		...
		decodedPacket.put(ProtocolAttribute.AUX_IS_USE_PW, Boolean.parseBoolean(attr.get(ConfigConstants.IsUsePw)));
		decodedPacket.put(ProtocolAttribute.CONTROLFIELD_FUNCTION_CODE, ControlField.PRM1_REQUEST_LEVEL_ONE_DATA);
		decodedPacket.put(ProtocolAttribute.AFN_FUNCTION,Afn.getAfnSymbol(Integer.parseInt(attr.get(ConfigConstants.AFN),16)));
		decodedPacket.put(ProtocolAttribute.SEQ_IS_NEED_CONFIRM, true);
		decodedPacket.put(ProtocolAttribute.DATAUNITIDENTIFY_FN, Integer.parseInt(attr.get(ConfigConstants.FN)));
		decodedPacket.put(ProtocolAttribute.DATAUNITIDENTIFY_PN, 0);
		SimpleBytes databody=
				new SimpleBytes(((String)attr.get(ConfigConstants.DataBody)).replaceAll("[^0-9a-fA-F]", ""),16,true);
		decodedPacket.put(ProtocolAttribute.CustomDataBody,databody );
